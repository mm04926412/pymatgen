...
class OrthorhombicSupercellTransform(AbstractTransformation):
    ...
    def apply_transformation(self, structure):
        ...
        start_len = len(struct)
        Q_abc = [np.linalg.norm(Q[0]),np.linalg.norm(Q[1]),np.linalg.norm(Q[2])]
        increments = (1,1,1)
        found_transform = False
        while not found_transform:
            new_increments = list(increments) #Deep copy
            shortest = np.argmin([i*j for i,j in zip(increments,Q_abc)]) #Return the shortest lattice parameter of Q post scaling
        ...
        struct = SupercellTransformation(scaling_matrix=cubic_upscale_approx).apply_transformation(struct) # Apply the computed integer supercell transformation
        return struct
